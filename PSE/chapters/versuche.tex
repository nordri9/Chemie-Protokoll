...
Im Falle stark metallischer Elemente wie Natrium liegt eine vollständige Dissoziation in Wasser vor:
\begin{equation}
    \ce{NaOH_{(s)} -> Na+_{(aq)} + OH^-_{(aq)}}
    \label{eq:naoh_dissoziation}
\end{equation}
Die hohe Konzentration an Hydroxidionen führt zu stark alkalischen pH-Werten.

Bei weniger basischen Metallhydroxiden wie Magnesiumhydroxid liegt ein Löslichkeitsgleichgewicht vor:
\begin{equation}
    \ce{Mg(OH)2_{(s)} <=> Mg^{2+}_{(aq)} + 2OH^-_{(aq)}}
    \label{eq:mgoh2_gleichgewicht}
\end{equation}
Die geringe Löslichkeit begrenzt die Hydroxidionenkonzentration und damit den pH-Wert.

Amphoteres Aluminiumhydroxid kann sowohl als Säure als auch als Base reagieren:
\begin{equation}
...
Für die nichtmetallischen Oxide bzw. Oxid-Hydroxide dominiert somit zunehmend die Bildung von Oxoniumionen in Wasser. Siliciumdioxid kann formal als schwache Kieselsäure beschrieben werden:
\begin{equation}
...
Phosphor-, Schwefel- und Chlorverbindungen bilden in Wasser Oxosäuren, die Protonen an das Lösungsmittel abgeben. Beispielhaft für Phosphorsäure:
\begin{equation}
    \ce{H3PO4_{(aq)} + H2O_{(l)} <=> H3O^+_{(aq)} + H2PO4^-_{(aq)}}
    \label{eq:phosphorsaure}
\end{equation}

Für Schwefelsäure gilt in der ersten Protolyse nahezu vollständige Dissoziation:
\begin{equation}
    \ce{H2SO4_{(aq)} + H2O_{(l)} -> H3O^+_{(aq)} + HSO4^-_{(aq)}}
    \label{eq:schwefelsaure}
\end{equation}

Chlorige Oxosäuren zeigen noch stärkere Säureeigenschaften:
\begin{equation}
    \ce{HClO3_{(aq)} + H2O_{(l)} -> H3O^+_{(aq)} + ClO3^-_{(aq)}}
    \label{eq:chloresaure}
\end{equation}

Der kontinuierliche Übergang von \eqref{eq:naoh_dissoziation} bis \eqref{eq:chloresaure} zeigt, dass mit zunehmendem Nichtmetallcharakter die Fähigkeit zur Bildung von Hydroxidionen abnimmt und stattdessen die Bildung von Oxoniumionen dominiert.
